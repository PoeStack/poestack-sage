...
export class GggHttpUtil {
  ...
  private reatelimiter: Record<string, GggRateLimiter> = {}
  private client = axios.create({ adapter: 'http' })

  private getOrCreateRateLimiter(token: string) {
    const existing = this.reatelimiter[token]
    if (existing) return existing

    this.reatelimiter[token] = new GggRateLimiter()
    return this.reatelimiter[token]
  }

  public get<T>(rateLimitKey: string, token: string, path: string): Observable<T> {
    return new Observable((observer) => {
      const rateLimiter = this.getOrCreateRateLimiter(token)
      const currentDelay = rateLimiter.getCurrentDelay(rateLimitKey)
      if (currentDelay > 0) {
        observer.error(new RateLimitError(currentDelay))
        observer.complete()
      } else {
        rateLimiter.recordRequestStart(rateLimitKey)
        this.client
          ...
          .then((response) => {
            ...
            rateLimiter.recordRequest(rateLimitKey, response.headers as unknown as any)
          ...
          .catch((error) => {
            ...
            if (error?.response?.headers) {
              rateLimiter.recordRequest(rateLimitKey, error.response.headers)
            }
