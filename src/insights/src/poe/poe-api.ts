...
export default class PoeApi {
  ...
  constructor(private readonly rateLimitService: PoeRateLimitService) {}

  public async fetchCurrentChangeIds(): Promise<string> {
    const res = await fetch('https://www.pathofexile.com/api/trade/data/change-ids', {
      headers: {
  ...
  public async fetchCharacter(
    ...
    const { data, rateLimitedForMs, status } = await this.fetch(url, bearerToken, {
      name
    })
  ...
  private async fetch(
    ...
    const rateLimitedForMs = this.rateLimitService.getRateLimiMs(token, variblePath)
    ...
    const rules = response.headers.get('x-rate-limit-rules')?.split(',')
    Logger.info('x-rate-limit', rateLimitInfo.policy, rules, rateLimitInfo.retryAfterMs)
    if (rateLimitInfo.policy && rules?.length > 0) {
      for (const rule of rules) {
        const limitArray = response.headers.get(`x-rate-limit-${rule}`).split(',')
        const stateArray = response.headers.get(`x-rate-limit-${rule}-state`).split(',')
