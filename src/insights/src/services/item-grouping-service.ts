...
export default class ItemGroupingService {
  ...
  public findOrCreateItemGroup(
    item: PoeApiItem
  ): { key: string; tag: string; hash: string } | null {
...
export class CompassGroupIdentifier implements ItemGroupIdentifier {
  ...
  constructor() {
    Object.entries(JSON.parse(fs.readFileSync('data/tft/tft-compas.json').toString())).forEach(
      (e) => {
  ...
  group(item: PoeApiItem): InternalGroup {
    ...
    if (baseType === 'charged compass') {
      const usesMod = item.enchantMods
        .filter((mod) => mod.includes(' uses remaining') || mod.includes(' use remaining'))?.[0]
        ?.replace(' uses remaining', '')
      const otherMods = item.enchantMods
        .filter((mod) => !mod.includes('uses remaining') && !mod.includes('use remaining'))
        .join(' ')
        .toLowerCase()
